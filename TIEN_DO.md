...
- [ ] **Chờ Khang duyệt**: 32 câu hỏi trong `data/eval/multihop_eval_set.json` (T016) — xem `CHECKLIST-GRAPHRAG-DUYET.md` mục E1. Chưa chặn tiến độ, chỉ cần duyệt trước khi dùng chính thức cho SC-001/Phase 4.

## 📍 Việc cần làm tiếp theo (đọc phần này trước khi bắt đầu phiên mới)

1. **T012 — `app/extraction/term_extractor.py`**: trích DEFINES (Article định nghĩa Term qua mẫu "... được hiểu là ...") và USES_TERM (Article dùng lại thuật ngữ đã định nghĩa). Rule-based trước (string-match tên thuật ngữ đã có), chỉ dùng Ollama khi rule-based không đủ (theo `data-model.md`, `constitution.md` Điều 1).
...
3. Sau T012/T013: cả `traversal.py` (đã build sẵn khả năng dùng DEFINES) sẽ có dữ liệu thật để traverse — nên chạy lại thử 1-2 câu hỏi qua `/chat` để xác nhận DEFINES thực sự hoạt động, không chỉ tồn tại trên lý thuyết.
4. Backfill embedding full 60k vẫn cần chạy xong trước khi Phase 4 (T017 Recall@k/MRR) có ý nghĩa đầy đủ — hiện chỉ ~3,700-4,000/60,679 đã embed.
5. Nếu muốn tăng tốc backfill: thử `--batch-size` lớn hơn trên GPU (64 hoặc 128) để kiểm chứng giả thuyết overhead — chưa làm trong phiên này.
