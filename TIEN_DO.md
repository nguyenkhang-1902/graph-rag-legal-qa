...
- [x] **GPU acceleration**: phát hiện máy có RTX 3050 nhưng torch cài bản CPU-only → cài lại `torch==2.13.0+cu126`, xác nhận CUDA hoạt động. Benchmark thật: ~5-8x nhanh hơn CPU (đang đo chính xác).
- [ ] **Còn lại Phase 3**: T012 (`term_extractor.py` — DEFINES/USES_TERM), T013 (`relation_llm.py` — AMENDS/SUPERSEDES/CONFLICTS_WITH). Backfill embedding đang chạy nền (GPU, batch tiếp theo sau subset CPU đầu) lúc kết thúc phiên này — kiểm tra tiến độ qua Neo4j (`count Article WHERE chroma_id IS NOT NULL`) hoặc chạy lại `python -m scripts.backfill_embeddings data/raw` để tiếp tục/mở rộng ra full 60k khi sẵn sàng.
