...
from scripts.eval_bhxh_qa import _score_free, _score_mc, _score_true_false
...
def test_qa_set_schema_valid():
    ...
    assert len(qs) >= 10
    for q in qs:
        assert q["type"] in {"true_false", "multiple_choice", "free_text"}
        assert q["gold_article_ids"]
        if q["type"] == "true_false":
            assert q["answer"] in {"Đúng", "Sai"}
